...
public class AccountDao {
	...
	public static int createAccount(Account a) {
		...
		try {
			...
			System.out.println(a.getWs_msg());
			System.out.println(a.getWs_acct_crdate());
			...
			ps.setString(7, a.getWs_msg());
			ps.setTimestamp(8, a.getWs_acct_crdate());
	...
	public static ArrayList<Account> readAllAccount() {
		...
		try {
			...
			while (rs.next()) {
				...
				a.setWs_msg(rs.getString(7));
				a.setWs_trxn_ts(rs.getTimestamp(8));
				account.add(a);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
...
//				a.setWs_acct_id(rs.getLong(2));
	...
	public static ArrayList<Account> readAllCustomerAccount(Account a) {
		...
		try {
			...
			while (rs.next()) {
				...
				acc.setWs_msg(rs.getString(7));
				acc.setWs_trxn_ts(rs.getTimestamp(8));
	...
	public static int updateAccountStatusUsingCustId(long cust_id) {
		...
		try {
			Connection con = DbConnection.getConnection();
			PreparedStatement ps = con.prepareStatement("update account set ws_acct_status = ?, ws_msg = ? where ws_cust_id = ?");
			ps.setString(1, "I");
