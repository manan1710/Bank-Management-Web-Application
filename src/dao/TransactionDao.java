...
public class TransactionDao {
	...
	public static int transactionDeposit(Transaction t) {
		...
		try {
			...
			PreparedStatement ps1 = con.prepareStatement(
					"update account set ws_acct_balance = ?,ws_msg = ?,ws_trxn_date = ?  where ws_acct_id = ?");
			ps1.setLong(1, new_balance);
			ps1.setString(2, "Amount Deposited : " + t.getWs_amt());
			ps1.setTimestamp(3, ts);
			ps1.setLong(4, t.getWs_src_acct_id());
	...
	public static int transactionWithdraw(Transaction t) {
		...
		try {
			...
			PreparedStatement ps1 = con.prepareStatement(
					"update account set ws_acct_balance = ?,ws_msg = ?,ws_trxn_date = ?  where ws_acct_id = ?");
			ps1.setLong(1, new_balance);
			ps1.setString(2, "Amount Withdrawn : " + t.getWs_amt());
			ps1.setTimestamp(3, ts);
			ps1.setLong(4, t.getWs_src_acct_id());
			int x = ps1.executeUpdate();
			if (x > 0) {
	...
	public static int transactionTransfer(Transaction src) {
		...
		try {
			...
			ps1.setString(2, "Amount Transferred : " + src.getWs_amt());
			ps1.setTimestamp(3, ts);
			ps1.setLong(4, src.getWs_src_acct_id());
			...
			if (xsrc > 0) {
				...
				ps1.setString(2, "Amount Received : " + src.getWs_amt());
				ps1.setTimestamp(3, ts);
				ps1.setLong(4, src.getWs_tgt_acct_id());
